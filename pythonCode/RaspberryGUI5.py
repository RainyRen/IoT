...
thu = ['Thu', '\n', 'Chinese','Math','Math','English', '\n', 'noon', '\n', 'Society','Science','Exercise']
fri = ['Fri', '\n', 'Chinese','Math','Math','English', '\n', 'noon', '\n', 'Society','Science','Exercise']
start_place_x = 19.0/395*r_width
start_place_y = 19.0/300*r_height
offset = 51
width_of_list = 8
...
def EM():
	...
	if emergency_button == 0:
		...
		payload = {"datapoints":[{"dataChnId":dataChnId8,"values":{"value":"0"}}, {"dataChnId":dataChnId9,"values":{"value":"0"}}]}
		post_to_mcs(payload)
		emergency_button = 0
...
def commadClassify(dataString):
	...
	if commandType == "Notification":
		...
		information.insert(len(information), notifiMessage)
...
def Danger_Message():
	...
	if emergency_button == 1 :
		if millis() % 200 > 100:
			...
			background_color = 'black'
		Screen_Refresh(background_color)
...
def Emergence_Switch():
	...
	emergency.pack()
	emergency.place(bordermode=OUTSIDE, x=333.0/395*r_width, y=262.0/300*r_height)
#------------------------------------------------------------------------------------------------------------------
def Menu_Button():
	menu_name = ""
	...
	mb = Menubutton ( root, text=menu_name, relief=RAISED, activebackground='green', cursor="hand2", width=int(15.0/395*r_width), height=2)
	mb.grid()
	...
	mb.menu.add_checkbutton ( label="", state=DISABLED)
	mb.menu.add_checkbutton ( label="", state=DISABLED)
	mb.menu.add_checkbutton ( label="Quit", onvalue=True, offvalue=False, variable=if_Quit)
	#mb.menu.add_checkbutton ( label="Quit", command=quit(), state=DISABLED)
	mb.pack()
	mb.place(bordermode=OUTSIDE, x=350.0/395*r_width, y=0)
#---------------------------------------------------------------------------------------------------------
def Record_Award():
	recordlist  = Listbox(height=r_height, width=width_of_list*4+2, bg=background_color, fg='white')
...
def Screen_Refresh(color_in):
	...
	screen.place(bordermode=OUTSIDE, x=-5, y=-5)              #change the position of the listbox
	Emergence_Switch()
#----------------------------------------------------------------------------------------------------------
def Announcement():
	global information
	...
	for item in information:
		...
		announce.pack()                                                    
		announce.place(bordermode=OUTSIDE, x=22.0/395*r_width, y=float(50+i*30)/300*r_height)
		i = i+1    	
#------------------------------------------------------------------------------------------------------------
...
def main_process():
	...
	global information_update
	global enter_time
	if (message_button.get()!=p_message_button.get() or task_button.get()!=p_task_button.get() or announcement_button.get()!=p_announcement_button.get() or record_button.get()!=p_record_button.get() or information_update == True or (millis()-enter_time > 2000 and message_button.get()==True and emergency_button==0) ):
		enter_time = millis()
		information_update = False
		Decorate()
		if message_button.get() == True:
